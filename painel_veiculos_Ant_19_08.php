...
<!-- SUBSTITUA AS FUNÇÕES DESKTOP E MOBILE NO painel_veiculos.php POR ESTAS: -->

<script>
// Variável para controlar o tipo de visualização
window.visualizacaoAtual = 'cards';

// Interceptar a função carregarOfertaVeiculos para adicionar o parâmetro de visualização
(function() {
    ...
    const interceptarCarregarOfertaVeiculos = setInterval(function() {
        if (typeof window.carregarOfertaVeiculos === 'function') {
            ...
            // Sobrescrever com nova função que adiciona o parâmetro
            window.carregarOfertaVeiculos = function() {
                ...
                const params = new URLSearchParams({
                    ...
                    preco: preco,
                    estado: estado,
                    visualizacao: window.visualizacaoAtual // Adiciona o parâmetro de visualização
                });
...
// ========== ATUALIZAR O LISTENER PARA AMBOS OS AVISOS ==========
document.addEventListener('click', function(e) {
    if (e.target.closest('.btn-fechar-aviso')) {
        console.log('🔄 Click detectado no botão X via delegação');
        e.preventDefault();
