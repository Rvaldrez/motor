...
// ✅ Captura o ID do usuário da sessão ou do POST (código de verificação)
$usuario_id = $_SESSION['usuario_id'] ?? ($_POST['usuario_id'] ?? null);

if (!$usuario_id) {
    echo json_encode(["success" => false, "message" => "Sessão inválida ou usuário não identificado."]);
    exit;
}

// 🔄 Atualiza o tipo do usuário para "Vendedor"
$mysqli->query("UPDATE usuarios SET tipo = 'vendedor' WHERE id = " . intval($usuario_id));
...
try {
    ...
    echo json_encode([
        ...
        "debug" => $e->getMessage() // 🔍 Ajuda no debug
    ]);
}
?>
